...
|---|---|---|---|---|
| **M1** | **甲器件最小全流程无人干预跑通**（主图+extract 子图+checkpoint，2–3 参数族 voff/u0[/rontr1]） | `python src/pipeline/run_agent.py --device jia` 全程无人干预；NRMSE ≤0.01%（卡12 基准 0.0000%/6 次仿真）；QA 全过；日志含预算计数；kill 后 `--resume` 续跑成功 | 未开始 | —— |
