...
| 拟合执行 | `view`(view_fields 五元组：filter/source/page/**selection**/prop) / `get_view_group_error` / `optimize`（同步阻塞，时长无明示上限）/ `start_optimize`+`get_job_status`+`cancel_job`+`dump_report`（异步；轮询超时建议 5–30min，API §12 原文）/ `save_sim_result` / `dump_view_group` | **核心差异**：优化器在 server 端（NanoSpice），不是"给参数返曲线"的 oracle；optimize 前置必须 view 且带 selection_string |
